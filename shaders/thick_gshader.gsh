...
void drawSegment(vec2 points[4], vec4 colors[4], float zValues[4])
{
    ...
    if (an1==0) an1 = 1;
    if (bn1==0) bn1 = 1;
    float length_a = pointSize / an1;
    float length_b = pointSize / bn1;
    ...
    if( dot( v0, v1 ) < -MiterLimit ) {
        ...
        if( dot( v0, n1 ) > 0 ) {
            ...
            gl_Position = vec4( ( p1 + pointSize * n0 ) / Viewport, zValues[1], 1.0 );
            EmitVertex();

            VertexOut.mTexCoord = vec2( 0, 0 );
            VertexOut.mColor = colors[1];
            gl_Position = vec4( ( p1 + pointSize * n1 ) / Viewport, zValues[1], 1.0 );
            EmitVertex();
